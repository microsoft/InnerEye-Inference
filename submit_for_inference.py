...
def create_run_config(azure_config: AzureConfig,
    ...
    :param environment_name: If specified, try to retrieve the existing Python environment with
    this name. If that     is not found, create one from the Conda files provided in
    `source_config`. This parameter is meant to be used when running inference for an existing
    model.
    :return: The configured script run.
    """
    # AzureML seems to sometimes expect the entry script path in Linux format, hence convert to
    # posix path
    entry_script_relative_path =\
        source_config.entry_script.relative_to(source_config.root_folder).as_posix()
    ...
    # account.
    run_config.source_directory_data_store =\
        workspace.datastores.get(WORKSPACE_DEFAULT_BLOB_STORE_NAME).name
